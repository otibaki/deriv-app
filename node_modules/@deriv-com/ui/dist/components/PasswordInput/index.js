import { jsxs as oe, jsx as S } from "react/jsx-runtime";
import { useEffect as ie, useState as T, useMemo as ne, useCallback as N } from "react";
import { s as M, D as O, a as U, R as C, b as se, z as m, c as te, B as le, M as $, d as V, e as H, A as ce, f as de, O as me, g as ue, S as B, E as he, h as X, i as be, j as ge, k as pe, l as ye, m as ke, n as I, o as fe } from "../../PasswordInput-53KGYo2D.js";
import { c as we } from "../../clsx-OuTLNxxd.js";
import { Input as ve } from "../Input/index.js";
import { calculateScore as je, isPasswordValid as ze } from "./PasswordUtils.js";
import { passwordRegex as xe, passwordErrorMessage as G, warningMessages as qe } from "./PasswordConstants.js";
import { EyeIcon as Me, EyeIconSlash as Ee } from "./PasswordIcon.js";
import { PasswordMeter as Se } from "./PasswordMeter.js";
class Ae {
  /*
  ...
  match({
    ...
    const a = [...this.getMatchesWithoutSeparator(e), ...this.getMatchesWithSeparator(e)], r = this.filterNoise(a);
    return M(r);
  }
  getMatchesWithSeparator(e) {
    const a = [], r = /^(\d{1,4})([\s/\\_.-])(\d{1,2})\2(\d{1,4})$/;
    for (let o = 0; o <= Math.abs(e.length - 6); o += 1)
      for (let n = o + 5; n <= o + 9 && !(n >= e.length); n += 1) {
        const s = e.slice(o, +n + 1 || 9e9), t = r.exec(s);
        if (t != null) {
          const l = this.mapIntegersToDayMonthYear([parseInt(t[1], 10), parseInt(t[3], 10), parseInt(t[4], 10)]);
          l != null && a.push({
            pattern: "date",
            token: s,
            i: o,
  ...
  getMatchesWithoutSeparator(e) {
    const a = [], r = /^\d{4,8}$/, o = (n) => Math.abs(n.year - C);
    for (let n = 0; n <= Math.abs(e.length - 4); n += 1)
      for (let s = n + 3; s <= n + 7 && !(s >= e.length); s += 1) {
        const t = e.slice(n, +s + 1 || 9e9);
        if (r.exec(t)) {
          const l = [], c = t.length;
          if (se[c].forEach(([h, g]) => {
            const p = this.mapIntegersToDayMonthYear([parseInt(t.slice(0, h), 10), parseInt(t.slice(h, g), 10), parseInt(t.slice(g), 10)]);
            p != null && l.push(p);
          }), l.length > 0) {
            let h = l[0], g = o(l[0]);
            l.slice(1).forEach((p) => {
  ...
  mapIntegersToDayMonthYear(e) {
    ...
    for (let n = 0, s = e.length; n < s; n += 1) {
      ...
      if (t > 99 && t < O || t > U)
        return null;
      t > 31 && (r += 1), t > 12 && (a += 1), t <= 0 && (o += 1);
    }
    return r >= 2 || a === 3 || o >= 2 ? null : this.getDayMonth(e);
  }
  // eslint-disable-next-line max-statements
  getDayMonth(e) {
    const a = [
    ...
    for (let o = 0; o < r; o += 1) {
      const [n, s] = a[o], t = this.mapIntegersToDayMonth(s);
      if (t != null)
        return {
          year: this.twoToFourDigitYear(n),
          month: t.month,
          day: t.day
        };
    }
  ...
  mapIntegersToDayMonth(e) {
    const a = [e, e.slice().reverse()];
    for (let r = 0; r < a.length; r += 1) {
      const o = a[r], n = o[0], s = o[1];
      if (n >= 1 && n <= 31 && s >= 1 && s <= 12)
        return {
          day: n,
          month: s
        };
...
const x = new Uint32Array(65536), _e = (i, e) => {
  const a = i.length, r = e.length, o = 1 << a - 1;
  let n = -1, s = 0, t = a, l = a;
  for (; l--; )
    x[i.charCodeAt(l)] |= 1 << l;
  for (l = 0; l < r; l++) {
    let c = x[e.charCodeAt(l)];
    const d = c | s;
    c |= (c & n) + n ^ n, s |= ~(c | n), n &= c, s & o && t++, n & o && t--, s = s << 1 | 1, n = n << 1 | ~(d | s), s &= d;
  }
  for (l = a; l--; )
    x[i.charCodeAt(l)] = 0;
  return t;
}, Te = (i, e) => {
  const a = e.length, r = i.length, o = [], n = [], s = Math.ceil(a / 32), t = Math.ceil(r / 32);
  for (let u = 0; u < s; u++)
    n[u] = -1, o[u] = 0;
  let l = 0;
  for (; l < t - 1; l++) {
    let u = 0, y = -1;
    const w = l * 32, k = Math.min(32, r) + w;
    for (let b = w; b < k; b++)
      x[i.charCodeAt(b)] |= 1 << b;
    for (let b = 0; b < a; b++) {
      const j = x[e.charCodeAt(b)], v = n[b / 32 | 0] >>> b & 1, z = o[b / 32 | 0] >>> b & 1, R = j | u, L = ((j | z) & y) + y ^ y | j | z;
      let E = u | ~(L | y), A = y & L;
      E >>> 31 ^ v && (n[b / 32 | 0] ^= 1 << b), A >>> 31 ^ z && (o[b / 32 | 0] ^= 1 << b), E = E << 1 | v, A = A << 1 | z, y = A | ~(R | E), u = E & R;
    }
    for (let b = w; b < k; b++)
      x[i.charCodeAt(b)] = 0;
  }
  let c = 0, d = -1;
  const h = l * 32, g = Math.min(32, r - h) + h;
  for (let u = h; u < g; u++)
    x[i.charCodeAt(u)] |= 1 << u;
  let p = r;
  for (let u = 0; u < a; u++) {
    const y = x[e.charCodeAt(u)], w = n[u / 32 | 0] >>> u & 1, k = o[u / 32 | 0] >>> u & 1, b = y | c, j = ((y | k) & d) + d ^ d | y | k;
    let v = c | ~(j | d), z = d & j;
    p += v >>> r - 1 & 1, p -= z >>> r - 1 & 1, v >>> 31 ^ w && (n[u / 32 | 0] ^= 1 << u), z >>> 31 ^ k && (o[u / 32 | 0] ^= 1 << u), v = v << 1 | w, z = z << 1 | k, d = z | ~(b | v), c = v & b;
  }
  for (let u = h; u < g; u++)
    x[i.charCodeAt(u)] = 0;
  return p;
  ...
  return e.length === 0 ? i.length : i.length <= 32 ? _e(i, e) : Te(i, e);
}, Ie = (i, e, a) => {
  const r = i.length <= e.length, o = i.length <= a;
  return r || o ? Math.ceil(i.length / 4) : a;
}, Pe = (i, e, a) => {
  let r = 0;
  const o = Object.keys(e).find((n) => {
    const s = Ie(i, n, a);
    if (Math.abs(i.length - n.length) > s)
      return !1;
    const t = De(i, n), l = t <= s;
    return l && (r = t), l;
  });
  return o ? {
    levenshteinDistance: r,
    levenshteinDistanceEntry: o
  } : {};
};
class Ce {
  constructor(e) {
  ...
  match({
    ...
    const a = e.split("").reverse().join("");
    return this.defaultMatch({
      password: a
    }).map((r) => ({
      ...r,
      token: r.token.split("").reverse().join(""),
      reversed: !0,
...
class Re {
  constructor({
    ...
    this.buffer = [], this.finalPasswords = [], this.substr = e, this.limit = a, this.trieRoot = r;
  }
  getAllPossibleSubsAtIndex(e) {
    const a = [];
    let r = this.trieRoot;
    for (let o = e; o < this.substr.length; o += 1) {
      const n = this.substr.charAt(o);
      if (r = r.getChild(n), !r)
        break;
      a.push(r);
    }
  ...
  helper({
    ...
    changes: n,
    lastSubLetter: s,
    consecutiveSubCount: t
  }) {
    if (this.finalPasswords.length >= this.limit)
      return;
    if (r === this.substr.length) {
      e === a && this.finalPasswords.push({
        password: this.buffer.join(""),
        changes: n
      });
      return;
    }
    const l = [...this.getAllPossibleSubsAtIndex(r)];
    let c = !1;
    for (let d = r + l.length - 1; d >= r; d -= 1) {
      const h = l[d - r];
      if (h.isTerminal()) {
        if (s === h.parents.join("") && t >= 3)
          continue;
        c = !0;
        const g = h.subs;
        for (const p of g) {
          ...
          if (this.helper({
            ...
            consecutiveSubCount: s === h.parents.join("") ? t + 1 : 1
          }), this.buffer.pop(), this.finalPasswords.length >= this.limit)
            return;
        }
      }
    }
    if (!e || !c) {
      const d = this.substr.charAt(r);
      this.buffer.push(d), this.helper({
        onlyFullSub: e,
        isFullSub: a && !c,
        index: r + 1,
        subIndex: o + 1,
...
const Le = (i, e, a) => new Re({
  substr: i,
  limit: e,
  trieRoot: a
}).getAll(), Ne = (i, e, a) => {
  const o = i.changes.filter((c) => c.i < e).reduce((c, d) => c - d.letter.length + d.substitution.length, e), n = i.changes.filter((c) => c.i >= e && c.i <= a), s = n.reduce((c, d) => c - d.letter.length + d.substitution.length, a - e + o), t = [], l = [];
  return n.forEach((c) => {
    t.findIndex((h) => h.letter === c.letter && h.substitution === c.substitution) < 0 && (t.push({
      letter: c.letter,
      substitution: c.substitution
    }), l.push(`${c.substitution} -> ${c.letter}`));
  }), {
    i: o,
    j: s,
    subs: t,
...
class Oe {
  ...
  isAlreadyIncluded(e, a) {
    return e.some((r) => Object.entries(r).every(([o, n]) => o === "subs" || n === a[o]));
  }
  match({
    password: e
  }) {
    const a = [], r = Le(e, m.l33tMaxSubstitutions, m.trieNodeRoot);
    let o = !1, n = !0;
    return r.forEach((s) => {
      if (o)
        return;
      ...
      n = !1, t.forEach((l) => {
        o || (o = l.i === 0 && l.j === e.length - 1);
        const c = Ne(s, l.i, l.j), d = e.slice(c.i, +c.j + 1 || 9e9), h = {
          ...l,
          l33t: !0,
          token: d,
          ...c
        }, g = this.isAlreadyIncluded(a, h);
        d.toLowerCase() !== l.matchedWord && !g && a.push(h);
      });
    }), a.filter((s) => s.token.length > 1);
  }
}
class Ue {
  constructor() {
    this.l33t = new Oe(this.defaultMatch), this.reverse = new Ce(this.defaultMatch);
  }
  match({
    password: e
  }) {
  ...
  defaultMatch({
    ...
    const r = [], o = e.length, n = e.toLowerCase();
    return Object.keys(m.rankedDictionaries).forEach((s) => {
      const t = m.rankedDictionaries[s], l = m.rankedDictionariesMaxWordSize[s], c = Math.min(l, o);
      for (let d = 0; d < o; d += 1) {
        const h = Math.min(d + c, o);
        for (let g = d; g < h; g += 1) {
          const p = n.slice(d, +g + 1 || 9e9), u = p in t;
          let y = {};
          const w = d === 0 && g === o - 1;
          m.useLevenshteinDistance && w && !u && a && (y = Pe(p, t, m.levenshteinThreshold));
          const k = Object.keys(y).length !== 0;
          if (u || k) {
            const b = k ? y.levenshteinDistanceEntry : p, j = t[b];
            r.push({
              pattern: "dictionary",
              i: d,
              j: g,
              token: e.slice(d, +g + 1 || 9e9),
              matchedWord: p,
              rank: j,
              dictionaryName: s,
              reversed: !1,
...
var q = {
  ...
  log10(i) {
    return i === 0 ? 0 : Math.log(i) / Math.log(10);
  },
  log2(i) {
  ...
  let e = le ** i.length;
  e === Number.POSITIVE_INFINITY && (e = Number.MAX_VALUE);
  let a;
  return i.length === 1 ? a = $ + 1 : a = V + 1, Math.max(e, a);
}, We = ({
  year: i,
  separator: e
}) => {
  let r = Math.max(Math.abs(i - C), H) * 365;
  return e && (r *= 4), r;
};
const Ye = (i) => {
  const e = i.split(""), a = e.filter((s) => s.match(me)).length, r = e.filter((s) => s.match(ue)).length;
  let o = 0;
  const n = Math.min(a, r);
  for (let s = 1; s <= n; s += 1)
    o += q.nCk(a + r, s);
  return o;
};
var $e = (i) => {
  const e = i.replace(ce, "");
  if (e.match(de) || e.toLowerCase() === e)
    return 1;
  const a = [B, he, X], r = a.length;
  for (let o = 0; o < r; o += 1) {
    const n = a[o];
    if (e.match(n))
      return 2;
...
const F = (i, e) => {
  let a = 0, r = i.indexOf(e);
  for (; r >= 0; )
    a += 1, r = i.indexOf(e, r + e.length);
  return a;
}, Ve = ({
  sub: i,
  token: e
}) => {
  const a = e.toLowerCase(), r = F(a, i.substitution), o = F(a, i.letter);
  return {
    subbedCount: r,
    unsubbedCount: o
  };
...
var He = ({
  ...
  const s = i, t = $e(o), l = He({
    l33t: a,
    subs: r,
    token: o
  }), c = e && 2 || 1;
  let d;
  return n === "diceware" ? d = 6 ** 5 / 2 : d = s * t * l * c, {
    baseGuesses: s,
    uppercaseVariations: t,
    l33tVariations: l,
    calculation: d
  };
}, Xe = ({
  ...
  switch (i) {
    case "recentYear":
      return Math.max(Math.abs(parseInt(e[0], 10) - C), H);
  }
  return 0;
}, Ke = ({
  baseGuesses: i,
  ...
  const a = i.charAt(0);
  let r = 0;
  return ["a", "A", "z", "Z", "0", "1", "9"].includes(a) ? r = 4 : a.match(/\d/) ? r = 10 : r = 26, e || (r *= 2), r * i.length;
};
const Qe = (i) => {
  let e = 0;
  return Object.keys(i).forEach((a) => {
    const r = i[a];
    e += r.filter((o) => !!o).length;
  }), e /= Object.entries(i).length, e;
}, Ze = ({
  token: i,
  graph: e,
  turns: a
}) => {
  const r = Object.keys(m.graphs[e]).length, o = Qe(m.graphs[e]);
  let n = 0;
  const s = i.length;
  for (let t = 2; t <= s; t += 1) {
    const l = Math.min(a, t - 1);
    for (let c = 1; c <= l; c += 1)
      n += q.nCk(t - 1, c - 1) * r * o ** c;
  }
  return n;
};
var ea = ({
  ...
  if (a) {
    ...
    if (a === 0 || n === 0)
      ...
      let s = 0;
      for (let t = 1; t <= Math.min(a, n); t += 1)
        s += q.nCk(a + n, t);
      o *= s;
    }
  }
...
const ra = (i, e) => {
  let a = 1;
  return i.token.length < e.length && (i.token.length === 1 ? a = $ : a = V), a;
}, W = {
  bruteforce: Fe,
  date: We,
  dictionary: Be,
  ...
  spatial: ea,
  separator: aa
}, oa = (i, e) => W[i] ? W[i](e) : m.matchers[i] && "scoring" in m.matchers[i] ? m.matchers[i].scoring(e) : 0;
var ia = (i, e) => {
  const a = {};
  if ("guesses" in i && i.guesses != null)
    return i;
  const r = ra(i, e), o = oa(i.pattern, i);
  let n = 0;
  typeof o == "number" ? n = o : i.pattern === "dictionary" && (n = o.calculation, a.baseGuesses = o.baseGuesses, a.uppercaseVariations = o.uppercaseVariations, a.l33tVariations = o.l33tVariations);
  const s = Math.max(n, r);
  return {
    ...i,
    ...a,
    guesses: s,
    guessesLog10: q.log10(s)
  };
};
const f = {
  password: "",
  ...
  makeBruteforceMatch(i, e) {
    return {
      pattern: "bruteforce",
      token: this.password.slice(i, +e + 1 || 9e9),
      i,
      j: e
    };
  },
  ...
  update(i, e) {
    const a = i.j, r = ia(i, this.password);
    let o = r.guesses;
    e > 1 && (o *= this.optimal.pi[r.i - 1][e - 1]);
    let n = q.factorial(e) * o;
    this.excludeAdditive || (n += ge ** (e - 1));
    let s = !1;
    Object.keys(this.optimal.g[a]).forEach((t) => {
      const l = this.optimal.g[a][t];
      parseInt(t, 10) <= e && l <= n && (s = !0);
    }), s || (this.optimal.g[a][e] = n, this.optimal.m[a][e] = r, this.optimal.pi[a][e] = o);
  },
  // helper: evaluate bruteforce matches ending at passwordCharIndex.
  bruteforceUpdate(i) {
    let e = this.makeBruteforceMatch(0, i);
    this.update(e, 1);
    for (let a = 1; a <= i; a += 1) {
      e = this.makeBruteforceMatch(a, i);
      const r = this.optimal.m[a - 1];
      Object.keys(r).forEach((o) => {
        r[o].pattern !== "bruteforce" && this.update(e, parseInt(o, 10) + 1);
      });
    }
  },
  ...
  unwind(i) {
    const e = [];
    let a = i - 1, r = 0, o = 1 / 0;
    const n = this.optimal.g[a];
    for (n && Object.keys(n).forEach((s) => {
      const t = n[s];
      t < o && (r = parseInt(s, 10), o = t);
    }); a >= 0; ) {
      const s = this.optimal.m[a][r];
      e.unshift(s), a = s.i - 1, r -= 1;
    }
    return e;
  }
};
var P = {
  ...
  //
  // ------------------------------------------------------------------------------
  mostGuessableMatchSequence(i, e, a = !1) {
    f.password = i, f.excludeAdditive = a;
    const r = i.length;
    let o = f.fillArray(r, "array");
    e.forEach((l) => {
      o[l.j].push(l);
    }), o = o.map((l) => l.sort((c, d) => c.i - d.i)), f.optimal = {
      ...
      m: f.fillArray(r, "object"),
      // same structure as optimal.m -- holds the product term Prod(m.guesses for m in sequence).
      // optimal.pi allows for fast (non-looping) updates to the minimization function.
      pi: f.fillArray(r, "object"),
      // same structure as optimal.m -- holds the overall metric.
      g: f.fillArray(r, "object")
    };
    for (let l = 0; l < r; l += 1)
      o[l].forEach((c) => {
        c.i > 0 ? Object.keys(f.optimal.m[c.i - 1]).forEach((d) => {
          f.update(c, parseInt(d, 10) + 1);
        }) : f.update(c, 1);
      }), f.bruteforceUpdate(l);
    const n = f.unwind(r), s = n.length, t = this.getGuesses(i, s);
    return {
      password: i,
      guesses: t,
      guessesLog10: q.log10(t),
      sequence: n
    };
  },
  getGuesses(i, e) {
    const a = i.length;
    let r = 0;
    return i.length === 0 ? r = 1 : r = f.optimal.g[a - 1][e], r;
  }
};
class na {
  // eslint-disable-next-line max-statements
  match({
    ...
    let o = 0;
    for (; o < e.length; ) {
      const s = this.getGreedyMatch(e, o), t = this.getLazyMatch(e, o);
      if (s == null)
        break;
      const {
        match: l,
        baseToken: c
      } = this.setMatchToken(s, t);
      if (l) {
        const d = l.index + l[0].length - 1, h = this.getBaseGuesses(c, a);
        r.push(this.normalizeMatch(c, d, l, h)), o = d + 1;
      }
    }
    return r.some((s) => s instanceof Promise) ? Promise.all(r) : r;
  }
  // eslint-disable-next-line max-params
  normalizeMatch(e, a, r, o) {
    const n = {
      pattern: "repeat",
      i: r.index,
      j: a,
      ...
      baseToken: e,
      baseGuesses: 0,
      repeatCount: r[0].length / e.length
    };
  ...
  getBaseGuesses(e, a) {
    const r = a.match(e);
    return r instanceof Promise ? r.then((n) => P.mostGuessableMatchSequence(e, n).guesses) : P.mostGuessableMatchSequence(e, r).guesses;
  }
}
class sa {
  constructor() {
  ...
  match({
    ...
    const n = e.length;
    for (let s = 1; s < n; s += 1) {
      const t = e.charCodeAt(s) - e.charCodeAt(s - 1);
      if (o == null && (o = t), t !== o) {
        const l = s - 1;
        this.update({
          i: r,
  ...
  update({
    ...
    if (a - e > 1 || Math.abs(r) === 1) {
      const s = Math.abs(r);
      if (s > 0 && s <= this.MAX_DELTA) {
        const t = o.slice(e, +a + 1 || 9e9), {
          sequenceName: l,
          sequenceSpace: c
        } = this.getSequence(t);
        return n.push({
  ...
  getSequence(e) {
    let a = "unicode", r = 26;
    return pe.test(e) ? (a = "lower", r = 26) : ye.test(e) ? (a = "upper", r = 26) : ke.test(e) && (a = "digits", r = 10), {
      sequenceName: a,
      sequenceSpace: r
    };
  }
...
class ta {
  ...
  match({
    ...
    const a = [];
    return Object.keys(m.graphs).forEach((r) => {
      const o = m.graphs[r];
      I(a, this.helper(e, o, r));
    }), M(a);
  }
  checkIfShifted(e, a, r) {
    return !e.includes("keypad") && // initial character is shifted
    this.SHIFTED_RX.test(a.charAt(r)) ? 1 : 0;
  }
  // eslint-disable-next-line complexity, max-statements
  helper(e, a, r) {
    let o;
    ...
    const t = e.length;
    for (; s < t - 1; ) {
      let l = s + 1, c = null, d = 0;
      for (o = this.checkIfShifted(r, e, s); ; ) {
        const h = e.charAt(l - 1), g = a[h] || [];
        let p = !1, u = -1, y = -1;
        if (l < t) {
          const w = e.charAt(l), k = g.length;
          for (let b = 0; b < k; b += 1) {
            const j = g[b];
            if (y += 1, j) {
              const v = j.indexOf(w);
              if (v !== -1) {
                p = !0, u = y, v === 1 && (o += 1), c !== u && (d += 1, c = u);
                break;
              }
            }
          }
...
const la = new RegExp(`[${fe.join("")}]`);
class _ {
  static getMostUsedSeparatorChar(e) {
    const a = [...e.split("").filter((o) => la.test(o)).reduce((o, n) => {
      const s = o.get(n);
      return s ? o.set(n, s + 1) : o.set(n, 1), o;
    }, /* @__PURE__ */ new Map()).entries()].sort(([o, n], [s, t]) => t - n);
    if (!a.length) return;
    const r = a[0];
    if (!(r[1] < 2))
      return r[0];
  ...
  match({
    ...
    const r = _.getMostUsedSeparatorChar(e);
    if (r === void 0) return a;
    const o = _.getSeparatorRegex(r);
    for (const n of e.matchAll(o)) {
...
class ca {
  ...
  match(e) {
    const a = [], r = [];
    return [...Object.keys(this.matchers), ...Object.keys(m.matchers)].forEach((n) => {
      if (!this.matchers[n] && !m.matchers[n])
        return;
      const s = this.matchers[n] ? this.matchers[n] : m.matchers[n].Matching, l = new s().match({
        password: e,
        omniMatch: this
      });
...
const K = 1, J = K * 60, Q = J * 60, Z = Q * 24, ee = Z * 31, ae = ee * 12, da = ae * 100, D = {
  second: K,
  minute: J,
  hour: Q,
  day: Z,
...
class ma {
  translate(e, a) {
    ...
    return o[r].replace("{base}", `${a}`);
  }
  estimateAttackTimes(e) {
    const a = {
      onlineThrottling100PerHour: e / 0.027777777777777776,
  ...
  guessesToScore(e) {
    return e < 1005 ? 0 : e < 1000005 ? 1 : e < 100000005 ? 2 : e < 1e10 + 5 ? 3 : 4;
  }
  displayTime(e) {
    let a = "centuries", r;
    const o = Object.keys(D), n = o.findIndex((s) => e < D[s]);
    return n > -1 && (a = o[n - 1], n !== 0 ? r = Math.round(e / D[a]) : a = "ltSecond"), this.translate(a, r);
  }
}
var ua = () => null, ha = () => ({
  warning: m.translations.warnings.dates,
  suggestions: [m.translations.suggestions.dates]
});
const ba = (i, e) => {
  let a = null;
  return e && !i.l33t && !i.reversed ? i.rank <= 10 ? a = m.translations.warnings.topTen : i.rank <= 100 ? a = m.translations.warnings.topHundred : a = m.translations.warnings.common : i.guessesLog10 <= 4 && (a = m.translations.warnings.similarToCommon), a;
}, ga = (i, e) => {
  let a = null;
  return e && (a = m.translations.warnings.wordByItself), a;
}, pa = (i, e) => e ? m.translations.warnings.namesByThemselves : m.translations.warnings.commonNames, ya = (i, e) => {
  let a = null;
  const r = i.dictionaryName, o = r === "lastnames" || r.toLowerCase().includes("firstnames");
  return r === "passwords" ? a = ba(i, e) : r.includes("wikipedia") ? a = ga(i, e) : o ? a = pa(i, e) : r === "userInputs" && (a = m.translations.warnings.userInputs), a;
};
var ka = (i, e) => {
  const a = ya(i, e), r = [], o = i.token;
  return o.match(B) ? r.push(m.translations.suggestions.capitalization) : o.match(X) && o.toLowerCase() !== o && r.push(m.translations.suggestions.allUppercase), i.reversed && i.token.length >= 4 && r.push(m.translations.suggestions.reverseWords), i.l33t && r.push(m.translations.suggestions.l33t), {
    warning: a,
    suggestions: r
  };
}, fa = (i) => i.regexName === "recentYear" ? {
  warning: m.translations.warnings.recentYears,
  suggestions: [m.translations.suggestions.recentYears, m.translations.suggestions.associatedYears]
} : {
  warning: null,
  suggestions: []
}, wa = (i) => {
  let e = m.translations.warnings.extendedRepeat;
  return i.baseToken.length === 1 && (e = m.translations.warnings.simpleRepeat), {
    warning: e,
    suggestions: [m.translations.suggestions.repeated]
  };
}, va = () => ({
  warning: m.translations.warnings.sequences,
  suggestions: [m.translations.suggestions.sequences]
}), ja = (i) => {
  let e = m.translations.warnings.keyPattern;
  return i.turns === 1 && (e = m.translations.warnings.straightRow), {
    warning: e,
    suggestions: [m.translations.suggestions.longerKeyboardPattern]
  };
}, za = () => null;
const Y = {
  warning: null,
...
class xa {
  ...
  setDefaultSuggestions() {
    this.defaultFeedback.suggestions.push(m.translations.suggestions.useWords, m.translations.suggestions.noNeed);
  }
  getFeedback(e, a) {
    if (a.length === 0)
    ...
    if (e > 2)
      return Y;
    const r = m.translations.suggestions.anotherWord, o = this.getLongestMatch(a);
    let n = this.getMatchFeedback(o, a.length === 1);
    return n != null ? n.suggestions.unshift(r) : n = {
      warning: null,
      suggestions: [r]
    }, n;
  }
...
const _a = {
  ...
  const a = je(i, e);
  let r = "";
  const o = { dictionary: { ..._a } };
  if (m.setOptions(o), !i)
    return { errorMessage: r, score: a };
  const { feedback: n } = Ea(i);
  return xe.isLengthValid.test(i) ? ze(i) ? e ? r = e : r = qe[n.warning] ?? "" : r = G.missingCharacter : r = G.invalidLength, { errorMessage: r, score: a };
}, Da = {
  0: "error",
  1: "error",
  2: "warning",
  ...
  onChange: r,
  value: o,
  customErrorMessage: n = "",
  ...s
}) => {
  ie(() => {
    g(n);
  }, [n]);
  const [t, l] = T(!1), [c, d] = T(!1), [h, g] = T(n), { errorMessage: p, score: u } = ne(
    () => Ta(o, h),
    [o, h]
  ), y = N(
    (k) => {
  ...
  return /* @__PURE__ */ oe(
    "div",
    {
      className: we("deriv-password", {
        "deriv-password--full": s.isFullWidth
      }),
      children: [
        /* @__PURE__ */ S(
          ve,
          {
            ...
            onChange: y,
            onBlur: w,
            variant: t ? Da[u] : "general",
            rightPlaceholder: /* @__PURE__ */ S(
              "button",
              {
                className: "deriv-password__icon",
                onClick: () => d(!c),
                type: "button",
                children: c ? /* @__PURE__ */ S(Me, {}) : /* @__PURE__ */ S(Ee, {})
              }
            ),
            ...s
          }
        ),
        !i && /* @__PURE__ */ S(Se, { score: u })
      ]
    }
  );
};
